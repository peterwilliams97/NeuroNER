...
class Dataset(object):
        ...
        for dataset_type in dataset_types:
            ...
            for token_sequence in tokens[dataset_type]:
                ...
                character_indices[dataset_type].append(
                    [[character_to_index.get(character, random.randint(1, max(self.index_to_character.keys())))
    ...
    def update_dataset(self, dataset_filepaths, dataset_types):
        '''
            ...
            Overwrites the data of type specified in dataset_types using the existing token_to_index,
            character_to_index, and label_to_index mappings.
        '''
        for dataset_type in dataset_types:
        ...
        (token_indices, label_indices, character_indices_padded, character_indices,
            token_lengths, characters, label_vector_indices) = self._convert_to_indices(dataset_types)
    ...
    def load_dataset(self, dataset_filepaths, parameters, token_to_vector=None):
        ...
        for character in (list(character_count['train'].keys()) + list(character_count['valid'].keys()) +
                          list(character_count['test'].keys()) + list(character_count['deploy'].keys())):
        ...
        label_count['all'] = {}
        for character in (list(label_count['train'].keys()) + list(label_count['valid'].keys()) +
                          list(label_count['test'].keys()) + list(label_count['deploy'].keys())):
        ...
        for label in labels_without_bio:
            ...
            for prefix in prefixes:
                pl = prefix + label
                if pl not in label_count['all']:
                    label_count['all'][pl] = 0
        label_count['all'] = utils.order_dictionary(label_count['all'], 'key', reverse=False)
        ...
        (token_indices, label_indices, character_indices_padded, character_indices, token_lengths,
            characters, label_vector_indices) = self._convert_to_indices(dataset_filepaths.keys())
        ...
        # labels = sorted(self.index_to_label.values())
        # parts = [(l[0], l[2:]) for l in labels]
        # from collections import defaultdict
        # parts_dict = defaultdict(set)
        # for prefix, name in parts:
        #     parts_dict[name].add(prefix)
        # for i, name in enumerate(sorted(parts_dict)):
        #     prefixes = sorted(parts_dict[name])
        #     print('%3d: %-20s %d %s' % (i, name, len(prefixes), prefixes))
        # assert False
