...
def assess_model(y_pred, y_true, labels, target_names, labels_with_o, target_names_with_o, dataset_type,
    ...
    classification_report = sklearn.metrics.classification_report(y_true, y_pred, labels=labels,
            target_names=target_names, sample_weight=None, digits=4)
...
def remap_labels(y_pred, y_true, dataset, evaluation_mode='bio'):
    ...
    new_y_pred = [remap_index[i] for i in y_pred]
    new_y_true = [remap_index[i] for i in y_true]
...
def evaluate_model(results, dataset, y_pred_all, y_true_all, stats_graph_folder, epoch_number,
    ...
    for dataset_type in ['train', 'valid', 'test']:
        ...
        for evaluation_mode in ['bio', 'token', 'binary']:
            y_pred, y_true, label_indices, label_names, label_indices_with_o, label_names_with_o =\
                remap_labels(y_pred_original, y_true_original, dataset,
                             evaluation_mode=evaluation_mode)
            result_update[dataset_type][evaluation_mode] = assess_model(
                            y_pred, y_true, label_indices, label_names, label_indices_with_o,
                            label_names_with_o,
                            dataset_type, stats_graph_folder, epoch_number, parameters,
                            evaluation_mode=evaluation_mode,
                            verbose=verbose)
    ...
    for dataset_type in ['train', 'valid', 'test']:
        ...
        shell_command = 'perl {0} < {1} > {2}'.format(conll_evaluation_script, output_filepaths[dataset_type],
