...
class NeuroNER(object):
    ...
    def _load_parameters(self, parameters_filepath, arguments={}, verbose=True):
        '''
            Load parameters from the ini file if specified, take into account any command line argument, and ensure
        ...
        if len(parameters_filepath) > 0:
            ...
            for k, v in nested_parameters.items():
                parameters.update(v)
        # Ensure that any arguments the specified in the command line overwrite parameters specified in the
        # parameter file
    ...
    def _check_parameter_compatiblity(self, parameters, dataset_filepaths):
        ...
        if parameters['train_model']:
            if 'train' not in dataset_filepaths or 'valid' not in dataset_filepaths:
                raise IOError("If train_model is set to True, both train and valid set must exist in the "
    ...
    def __init__(self,
        ...
        session_conf = tf.ConfigProto(
            ...
            # automatically choose an existing and supported device to run the operations in case the
    ...
    def fit(self):
        ...
        with open(os.path.join(model_folder, 'parameters.ini'), 'w') as f:
            conf_parameters.write(f)
        with open(os.path.join(model_folder, 'dataset.pickle'), 'wb') as f:
        ...
        with codecs.open(token_list_file_path, 'w', 'UTF-8') as f:
            for token_index in range(dataset.vocabulary_size):
                f.write('{0}\n'.format(dataset.index_to_token[token_index]))

        with codecs.open(character_list_file_path, 'w', 'UTF-8') as f:
